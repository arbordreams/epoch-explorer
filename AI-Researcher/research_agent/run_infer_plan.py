...
class InnoFlow(FlowModule):
    def __init__(self, cache_path: str, log_path: Union[str, None, MetaChainLogger] = None, model: str = COMPLETION_MODEL, code_env: DockerEnv = None, web_env: BrowserEnv = None, file_env: RequestsMarkdownBrowser = None):
        ...
        self.prepare_agent = AgentModule(get_prepare_agent(model=CHEEP_MODEL, reasoning_effort="low", code_env=code_env), self.client, cache_path)
        self.download_papaer = ToolModule(download_arxiv_source_by_title, cache_path)
        self.coding_plan_agent = AgentModule(get_coding_plan_agent(model=COMPLETION_MODEL, reasoning_effort="high", code_env=code_env), self.client, cache_path)
        self.ml_agent = AgentModule(get_ml_agent(model=COMPLETION_MODEL, code_env=code_env), self.client, cache_path)
        self.judge_agent = AgentModule(get_judge_agent(model=COMPLETION_MODEL, reasoning_effort="high", code_env=code_env, web_env=web_env, file_env=file_env), self.client, cache_path)
        self.survey_agent = AgentModule(get_survey_agent(model=COMPLETION_MODEL, reasoning_effort="high", file_env=file_env, code_env=code_env), self.client, cache_path)
        self.exp_analyser = AgentModule(get_exp_analyser_agent(model=COMPLETION_MODEL, reasoning_effort="high", file_env=file_env, code_env=code_env), self.client, cache_path)
...
5. Integration
   ...
   - Auto-detect GPU memory and capabilities using `torch.cuda.get_device_properties()`
   - Dynamically adjust batch sizes and model hyperparameters based on available GPU memory to maximize utilization
   - Enable optimizations: mixed precision (`autocast`), TF32 for compatible GPUs, `cudnn.benchmark`
   - Optimize DataLoaders with `persistent_workers`, `prefetch_factor`, and appropriate `num_workers`
   - Keep OOM fallback: reduce batch size/hyperparameters and retry if OOM occurs; let the implementation choose specific values based on detected GPU capabilities (do not hard-code exact batch sizes or hyperparameters)
